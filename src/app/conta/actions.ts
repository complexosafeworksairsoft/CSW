...
export async function addEquipmentAction(
  ...
  const result = await addEquipment(operator.id, {
    ...
    optics: readCatalogSingle(formData, "optics", RED_DOT_OPTICS),
    scopes: readCatalogSingle(formData, "scopes", SCOPE_OPTICS),
    lightsLasers: readCatalogSingle(formData, "lightsLasers", LIGHTS_LASERS),
    muzzleDevices: readCatalogSingle(formData, "muzzleDevices", MUZZLE_DEVICES),
    stocks: readCatalogSingle(formData, "stocks", STOCKS),
...
/** Edits one of the account owner's own equipment items — same idea as addEquipmentAction, but updates an existing row (scoped to their own operatorId, resolved by userId) instead of inserting a new one. */
